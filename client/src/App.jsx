...
import { LayoutDashboard, Map, User, Settings as SettingsIcon, LogOut, Activity, RefreshCw, Menu, X, ChevronRight } from 'lucide-react'
...
const Sidebar = ({ user, handleLogout, handleSwitchRole, isOpen, onClose }) => {
  ...
  const sidebarContent = (
    <div className="flex flex-col gap-10 h-full">
      ...
      <div className="mt-auto pt-8 text-center md:hidden">
         <p className="text-[8px] font-mono text-white/20 uppercase tracking-[0.5em]">BringIt Studio Architecture v2.0</p>
      </div>
    </div>
  );
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
              onClick={onClose}
              className="fixed inset-0 bg-bg-deep/60 backdrop-blur-sm z-[100] md:hidden"
            />
            {/* Drawer */}
            <motion.div
              initial={{ x: "-100%" }}
              ...
              className="fixed inset-y-0 left-0 w-[300px] bg-bg-card border-r border-white/10 p-8 z-[110] md:hidden shadow-2xl overflow-y-auto"
            >
  ...
  return (
    ...
    >
      ...
      />
       ...
       <main className={cn("flex-1 relative z-10", isAuthPage ? "flex items-center justify-center p-0" : "pl-0 md:pl-[280px] pt-24 md:pt-12 pb-24 md:pb-0 p-4 md:p-12")}>
          {!isAuthPage && user && (
